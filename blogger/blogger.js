  ...
  try {
    ...
    posts.forEach( async (post, index) => {
      ...
      const image =
        ...
        if (imdbId) {
            try {
                const tmdbResponse = await fetch(`https://api.themoviedb.org/3/find/${imdbId}?api_key=${tmdbApiKey}&external_source=imdb_id`);
                        ...
                        <div class="w-full" style="flex: 0 0 100%" id="slide-0">
                                <div class="relative h-[400px] md:h-[500px] flex">
                                        ...
                                        <div class="absolute inset-0 z-0">
                                                <img src="${movieDetails.backdrop_path ? `https://image.tmdb.org/t/p/original${movieDetails.backdrop_path}` : image}" alt="${title}" class="w-full h-full object-cover opacity-30">
                                        ...
                                        <div class="container mx-auto flex items-center relative z-10">
                                                <div class="hidden md:block w-1/3 p-6">
                                                        <img src="${movieDetails.poster_path ? `https://image.tmdb.org/t/p/w500${movieDetails.poster_path}` : image}" alt="${title}" class="w-full max-w-[300px] rounded-lg shadow-2xl hover:scale-105 transition-transform duration-300">
                                                </div>
                                                ...
                                                <div class="w-full md:w-2/3 p-6 md:p-10">
                                                        ...
                                                        <div class="flex flex-wrap items-center gap-4 mb-4">
                                                                ...
                                                                <span class="text-gray-300 text-sm">
                                                                        ${language === "hi" ? "🇮🇳 Bollywood" : language === "en" ? "🌟 Hollywood" : "🎥 International"}
                                                                </span>
                                                        </div>
